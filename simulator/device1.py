...
class Device():
    def __init__(self, uid, slaveId=1, rutinRead=5, rutinSend=60):
        ...
        self.alert = {
            "DI1": 1, "DI1MSG": "DEPO SAMANDIRA SEVIYE BILGISI DURUM DEGISTIRDI",
            "DI2": 1, "DI2MSG": "ELEKTRIK AKIM BILGISI DURUM DEGISTRDI",
            "DI3": 0, "DI3MSG": "",
            "DI4": 0, "DI4MSG": "",
            "DI5": 0, "DI5MSG": "",
            "DO1": 1, "DO1MSG": "",
            ...
            "AO3": 0, "AO3MSG": "",
            "AO4": 0, "AO4MSG": "",
            "AO5": 1, "AO5MSG": "DEPO CIHAZI CALISIR DURUMDA"
        }
        ...
        while (1):
            try:
                ...
                msg = {"timestamp": timestamp, "UID":self.UID, "MSGID":self.MSGID, "SLAVEID":self.SLAVEID,
                        "IO": {"DI1":self.IO.get("DI1"), "DI2":self.IO.get("DI2"), "DI3":self.IO.get("DI3"),
...
device = Device(uid="1234567891", slaveId=1)
...
async def func(request:Request):
    a=device.IO #burda güncel ioları alıyo
    data = {"samandira cihaz durumu : ": a}
    return data
